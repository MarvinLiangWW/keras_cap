...
import random
np.random.seed(40)
col_meaning=['year','血小板分布宽度','嗜碱细胞绝对值','红细胞总数','紅細胞分布宽度标准差','平均红细胞体积','中性细胞绝对值',
             '平均红细胞血红蛋白浓度','血小板总数','血红蛋白','淋巴细胞绝对值','白细胞总数','单核细胞百分比','单核细胞绝对值',
             '淋巴细胞百分比','嗜碱细胞百分比','嗜酸细胞百分比','血小板压积','大血小板比率','红细胞压积','红细胞分布宽度变异系数',
             '中性细胞百分比','平均红细胞血红蛋白含量','嗜酸细胞绝对值','血小板平均体积','C反应蛋白','白蛋白','丙氨酸氨基转移酶',
             '天门冬氨酸氨基转移酶']
...
def etc_gbdt(X,y,mode):
    ...
    for i in range(0, len(indict1)):
        ...
        f.write('%d(%f)\n' % (indict1[i], importance1[indict1[i]]))
...
if __name__ == '__main__':
    print("main")
    X, y =fun('number_age_col55tran.csv', 'Category.csv')
