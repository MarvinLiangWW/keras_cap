...
def temp_lstm(f_in='5_day_50_check.csv'):
    ...
    for epoch in range(nb_epochs):
        ...
        if acc>best_acc:
            ...
            best_epoch=epoch
    plot(range(0,nb_epochs+1),acc_list,label='temp')
...
def para_prediction():
    ...
    x_train, x_test, x_validation, y_train, y_test, y_validation = get_train_test_data(X, y, nb_x_train, nb_x_test,
                                                                                       nb_x_validation)
    probability_validation = (sum(y_validation) - len(y_validation)) / len(y_validation)
    probability_test = (sum(y_test) - len(y_test)) / len(y_test)
    print('probability_validation:', probability_validation)
    ...
    print(results)
    plot(range(0, nb_epochs + 1), acc_list,label='para')
...
if __name__ =='__main__':
    ...
    plot(range(0, nb_epochs + 1), acc_list,label='merge')
    results = ""
    for i, acc in enumerate(acc_list):
        if acc > 0.730:
            if acc > 0.745:
    ...
    print("top-K mean: %.3f" % np.mean(np.array(acc_list[:10])))
    legend()
    savefig('merge_dataset_%d_'%(data_version)+time.strftime('%Y_%m_%d_%H_%M_%S.jpeg',time.localtime(time.time())))
